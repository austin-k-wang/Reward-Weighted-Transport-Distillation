...
class SanaSprintPipeline(nn.Module):
    def __init__(
        ...
        """Build a SANA-Sprint inference pipeline on a selected device.

        Args:
            config: Path to the SANA-Sprint YAML configuration.
            device: Device used for the VAE, text encoder, and diffusion model.
                When omitted, defaults to ``cuda:0`` if CUDA is available and
                CPU otherwise.

        Returns:
            Nothing. The initialized module owns all inference components.
        """
        super().__init__()
    ...
    def set_progress_bar_config(self, *, disable: bool = False):
        """Configure whether the internal denoising progress bar is hidden.

        Args:
            disable: If true, suppress the per-call tqdm denoising bar. This
                is useful when an outer evaluation progress bar is active.

        Returns:
            Nothing. The setting applies to subsequent pipeline calls.
        """
        ...
        for prompt in prompts:
            ...
            with torch.no_grad():
                ...
                for i, t in tqdm(
                    ...
                    disable=self.progress_bar_disabled,
                ):
