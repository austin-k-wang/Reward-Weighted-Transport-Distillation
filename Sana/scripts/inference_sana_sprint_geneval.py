...
def set_env(seed=0, latent_size=256):
    """Configure deterministic process state before GenEval generation.

    Args:
        seed: Base seed used for process-global libraries.
        latent_size: Legacy latent width used by the initialization warmup.

    Returns:
        Nothing. Python, NumPy, and Torch deterministic state is configured.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def visualize(sample_steps, cfg_scale):
    official_generator = None
    if args.seed_mode == "official":
        official_generator = torch.Generator(device=device).manual_seed(args.seed)
        advance_official_geneval_generator(
            official_generator,
            prompt_count=args.start_index,
            batch_size=batch_size,
            latent_channels=config.vae.vae_latent_dim,
    ...
    explicit_timesteps = validate_geneval_timesteps(sample_steps, args.timesteps)
    scheduler.set_timesteps(
        num_inference_steps=sample_steps,
        max_timesteps=args.max_timesteps,
        intermediate_timesteps=args.intermediate_timesteps,
        timesteps=explicit_timesteps,
    )
    ...
    for index, metadata in tqdm(list(enumerate(metadatas)), desc=tqdm_desc, position=args.gpu_id, leave=True):
        ...
        with torch.no_grad():
            ...
            for _ in range((args.n_samples + batch_size - 1) // batch_size):
                current_batch_size = min(batch_size, args.n_samples - sample_count)
                # Generate images
                prompts, hw, ar = (
                    [],
                    ...
                    torch.tensor([[1.0]], device=device).repeat(current_batch_size, 1),
                )
                ...
                # check exists
                chunk_paths = [
                    os.path.join(sample_path, f"{sample_index:05}.png")
                    for sample_index in range(
                        sample_count,
                        sample_count + current_batch_size,
                    )
                ]
                if all(os.path.exists(path) for path in chunk_paths):
                ...
                with torch.no_grad():
                    ...
                    if official_generator is not None:
                        ...
                        for batch_index in range(n):
                            sample_seed = geneval_sample_seed(
                                args.seed,
                                index,
                                sample_count + batch_index,
                            )
                        ...
                        latents = torch.cat(latent_samples, dim=0)
...
class SanaInference(SanaConfig):
    ...
    adapter_path: Optional[str] = field(
        default=None,
        metadata={"help": "Optional PEFT adapter directory to merge for evaluation"},
    )
    sample_nums: int = 553
    cfg_scale: float = 4.5
    ...
    seed_mode: str = field(
        default="invariant",
        metadata={"help": "Noise stream mode: invariant or official"},
    )
    step: int = -1
    add_label: str = ""
...
if __name__ == "__main__":
    ...
    if args.seed_mode not in {"invariant", "official"}:
        raise ValueError(
            f"seed_mode must be 'invariant' or 'official', got {args.seed_mode!r}"
        )
    ...
    if args.adapter_path:
        ...
        nested_adapter = os.path.join(adapter_path, "adapter")
        if os.path.isfile(os.path.join(nested_adapter, "adapter_config.json")):
            adapter_path = nested_adapter
        if not os.path.isfile(os.path.join(adapter_path, "adapter_config.json")):
            raise FileNotFoundError(
                f"PEFT adapter_config.json not found under {args.adapter_path}"
            )
    ...
    # dataset
    metadata_path = os.environ.get("GENEVAL_DATA_URL")
    if not metadata_path or not os.path.isfile(metadata_path):
        raise FileNotFoundError(
            "GENEVAL_DATA_URL must point to a local evaluation_metadata.jsonl"
        )
    all_metadatas = load_jsonl(metadata_path)
    metadatas = all_metadatas[args.start_index : args.end_index]
